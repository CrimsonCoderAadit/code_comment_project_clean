...
# Copy all the functions from build_synthetic_graphs.py
...
def main():
    # MODIFIED PATHS for LARGE synthetic data
    input_csv = "data/large_synthetic_test_data.csv"  # <-- Changed to large synthetic data
    output_dir = "data/large_synthetic_graphs"        # <-- Separate output directory
    embeddings_path = "data/simplified_embeddings.pkl"  # Use existing embeddings
    
    # Check if input file exists
    if not os.path.exists(input_csv):
        print(f"❌ Large synthetic data file not found: {input_csv}")
        print("💡 Run 'python src/generate_large_test_data.py' first!")
        return
    ...
    print(f"🤖 Processing LARGE SYNTHETIC data for comprehensive model testing")
    print(f"📂 Loading large synthetic dataset: {input_csv}")
    ...
    print(f"📊 Total large synthetic samples: {len(df)}")
    ...
    # Process large synthetic data
    valid_rows = []
    for i, row in tqdm(df.iterrows(), total=len(df), desc="Preprocessing large synthetic data"):
    ...
    print(f"📈 Processing {len(valid_rows)} valid large synthetic samples...")
    
    # Build graphs for large synthetic data
    saved = 0
    skipped = 0
    
    for i, row, label in tqdm(valid_rows, desc="Building graphs for large synthetic data"):
        ...
        # Save graph with large_synthetic prefix
        torch.save(graph, os.path.join(output_dir, f"large_synthetic_graph_{i}.pt"))
        saved += 1
    
    print(f"\n✅ Large synthetic graph building complete!")
    print(f"📊 Saved: {saved} large synthetic graphs")
    print(f"⚠️  Skipped: {skipped} large synthetic graphs") 
    print(f"🎯 Using existing embeddings with {embedder.get_feature_dim()} dimensions")
    print(f"💾 Graphs saved to: {output_dir}")
    print(f"\n🚀 Ready for comprehensive model testing! Run: python src/analyze_large.py")
